...
双主 AXI4-Lite 互连（`axil_interconnect`）+ 11 个从口：

| 从口 | 基地址 | 外设 |
|---|---|---|
| RAM | `0x00000000` | 64 KB 指令/数据 RAM（`axil_ram`，字节写使能） |
| BOOT | `0x10000000` | `boot_ctrl` CPU 复位控制 |
| IRQ | `0x20000000` | `irq_ctrl` 中断控制器（16 源聚合） |
| I2C0-3 | `0x30000000`+ | `i2c_master_axil` ×4 |
| UART0-1 | `0x40000000`+ | `uart_axil` ×2 |
| APB0 | `0x50000000` | GPIO + TIMER0（经 `axil2apb` 桥） |
| APB1 | `0x60000000` | CTRL + TIMER1（经 `axil2apb` 桥） |
...
- **功能**：双主 AXI-Lite SoC，11 从口外设全部可访问；RT-Thread 8 任务并发自测全部 PASS；
- **性能**：
  - 固件下载吞吐 ≈ 39.9 MB/s；
  - APB 路径事务 3 拍（30 ns）；
